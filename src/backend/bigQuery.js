...
const credentials = JSON.parse(Buffer.from(process.env.GOOGLE_CREDENTIALS_FILE, 'base64').toString('utf8'));
...
export async function querySolanaValidatorData(pubkeys, startDate, endDate) {
  ...
  const query = `SELECT * FROM \`bigquery-public-data.crypto_solana_mainnet_us.Block Rewards\` WHERE pubkey IN (${pubkeyList}) AND block_timestamp >= '${startDate}' AND block_timestamp < '${endDate}' ORDER BY block_timestamp DESC;`;
