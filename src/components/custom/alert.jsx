...
export const CustomNetworkAlert = ({ chainId, address, txType, setTxType }) => {
  return (
    <div className="relative">
      {/* Pixel Flicker Corners with pointer-events: none */}
      <div className="absolute top-0 left-0 w-4 h-4 bg-black flicker" style={{ pointerEvents: 'none' }} />
      <div className="absolute top-0 right-0 w-4 h-4 bg-black flicker" style={{ pointerEvents: 'none' }} />
      <div className="absolute bottom-0 left-0 w-4 h-4 bg-black flicker" style={{ pointerEvents: 'none' }} />
      <div className="absolute bottom-0 right-0 w-4 h-4 bg-black flicker" style={{ pointerEvents: 'none' }} />
              ...
              >
                <div className="flex items-center px-2 py-2 bg-gray-100 hover:bg-yellow-100 transition">
                  <input
                    ...
                    checked={txType === item.id}
                    onChange={() => setTxType(item.id)}
                  />
                  <label
